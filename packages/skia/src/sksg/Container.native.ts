...
import { workerProducer } from "./WorkerProducer";
...
/**
 * Recording renderer with a dedicated producer thread: the scene is recorded
 * on the worker runtime (see WorkerProducer) and the SkiaRecordingView only
 * presents. The UI runtime runs one cheap mapper per canvas that bumps a
 * version number when an animated value or the target changes; the worker
 * polls that number.
 */
class WorkerContainer extends Container {
  private mapperId: number | null = null;
  private target: SharedValue<SkDeferredTargetInfo | null>;
  private version: SharedValue<number>;
  ...
  constructor(
    ...
    this.version = Rea.makeMutable(0);
    workerProducer.register(nativeId, this.target, this.version);
  }
  ...
  unmount() {
    ...
    workerProducer.unregister(this.nativeId);
  }
  ...
  redraw() {
    ...
    const sharedValues = recorder.getSharedValues();
    const sharedRecorder = recorder.getRecorder();
    const { version, target } = this;
    // Change detection only; nothing is drawn on the UI runtime.
    this.mapperId = Rea.startMapper(
      () => {
        "worklet";
    ...
    // Hands the scene to the producer thread, which draws it as soon as the
    // view has a target, and again whenever the version moves.
    workerProducer.publish(
      this.Skia,
      this.nativeId,
      sharedRecorder,
      sharedValues
...
export const createContainer = (
  ...
  if (useRecording && HAS_WORKLET_RUNTIMES && nativeId !== -1) {
    return new WorkerContainer(Skia, nativeId);
  }
  if (HAS_REANIMATED_4 && nativeId !== -1) {
    // Picture renderer: the scene is drawn into an SkPicture on the UI
    // runtime and the SkiaPictureView replays it.
